...
// I2S master channel that provides MCLK/BCLK/WS to the ES8311 and reads its
// ADC output on DIN. The P4 is the I2S master, the ES8311 is the slave.
...
// streams onto the I2S bus set up above.
static void codec_mic_init(void)
{
    ...
    i2s_chan_handle_t rx = i2s_rx_init();
    ...
    es8311_codec_cfg_t es_cfg = {
        .ctrl_if = ctrl_if,
        .gpio_if = gpio_if,
        .codec_mode = ESP_CODEC_DEV_WORK_MODE_ADC, // record only, no playback
        .master_mode = false,                      // ES8311 is the I2S slave
        .use_mclk = true,
        .digital_mic = false,                      // analog mic on MIC1
        .pa_pin = -1,                              // no power amp for recording
        .mclk_div = 256,
    };
    ...
    ESP_LOGI(TAG, "ES8311 microphone ready (%d Hz, gain %d dB)", SAMPLE_RATE, CONFIG_P4_MIC_GAIN_DB);
}
...
void audio_pipeline_start(audio_clip_ready_cb_t on_clip_ready)
{
    ...
    // DET_MODE_90 (default) missed wake words too often in testing; 95 raises
    // the detection probability at the cost of a somewhat higher false-alarm
    // rate, which is the right tradeoff for a device that requires a second
    // spoken command after waking anyway.
