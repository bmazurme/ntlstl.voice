...
- `router.ts` — все три эндпоинта и их хелперы (whisper/ffmpeg subprocess, запрос к Ollama, вызов HA-сервисов).
- `config.ts` — все переменные окружения (`PORT`, `WHISPER_*`, `OLLAMA_*`, `HA_*`, `SYSTEM_PROMPT_PATH`) в одном месте.
